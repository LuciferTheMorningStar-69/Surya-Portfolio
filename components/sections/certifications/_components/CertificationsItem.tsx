...
import { type Certification, type CertificationDate } from "@/constant";
...
const formatDate = (date: CertificationDate) => {
  return `${date.mm.slice(0, 3)} ${date.yyyy}`;
};

export const CertificationsItem = ({ item, index }: CertificationsItemProps) => {
  const ref = useRef<HTMLDivElement>(null);
  const isInView = useInView(ref, { once: true, margin: "-60px" });

    const dateRange = "Completed";
    ...
    >
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between border-b border-neutral-800/60 pb-4">
        <div className="flex flex-col gap-1">
          <div className="inline-flex items-center gap-2 font-heading text-xl font-bold uppercase tracking-tight text-white sm:text-2xl">
